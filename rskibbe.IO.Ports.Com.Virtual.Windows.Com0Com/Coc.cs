...
public class Coc : VirtualComPortsBase
{
    ...
    public override async Task<IVirtualComPortRegistration> CreateVirtualPortsAsync()
    {
        var pattern = "COM#";
        ...
        var registration = CocPortsRegistration.FromBinaryResponse(lines);
    ...
    public override async Task<IVirtualComPortRegistration> CreateVirtualPortsAsync(byte portIdA, byte portIdB)
    {
        ...
        var registration = CocPortsRegistration.FromBinaryResponse(lines);
        registration.ComPorts = new VirtualComPortPair(portNameA, portNameB);
        return registration;
    }
    ...
    public override async Task RemoveVirtualPortsAsync(string portNameAOrB)
    {
        ...
        await RemoveVirtualPortsAsync(((CocPortsRegistration)registrationToRemove).Id);
    }
    ...
    public override async Task RemoveVirtualPortsAsync(int regId)
    {
        var portARemoved = false;
        var portBRemoved = false;
        var lines = await ExecuteBinaryAsync($"--silent remove {regId}");
        foreach (var line in lines)
        {
            if (line.StartsWith($"Removed CNCA{regId}"))
            {
                portARemoved = true;
            }
            else if (line.StartsWith($"Removed CNCB{regId}"))
            {
                portBRemoved = true;
                break;
            }
        ...
        if (!bothPortsRemoved)
            throw new ComPortsRemovalException($"The port pair with registration id {regId} could not be removed");
    }
    ...
    public override async Task RemoveAllVirtualPortsAsync()
    {
        ...
        foreach (var registration in registrations)
            await RemoveVirtualPortsAsync(((CocPortsRegistration)registration).Id);
    }
    ...
    /// </summary>
    public override async Task<IEnumerable<IVirtualComPortRegistration>> ListVirtualPortRegistrationsAsync()
    {
        var list = new List<CocPortsRegistration>();
        ...
        {
            var registration = new CocPortsRegistration(idToLines.Key);
            var comPortPair = new VirtualComPortPair();
            for (var i = 0; i <= 1; i++)
            {
    ...
    /// </summary>
    public async Task<VirtualComPortPair> GetVirtualComPortsByRegistrationIdAsync(int id)
    {
        var comPortPair = new VirtualComPortPair();
